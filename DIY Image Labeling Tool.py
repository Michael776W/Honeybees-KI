...
class LabelingTool:
    def __init__(self, root):
        ...
        self.class_buttons = ['Biene', 'Varroa', 'Pollen_Gelb', 'Pollen_Orange', 'Pollen_Rot', 'Pollen_Braun']
    ...
    def on_mouse_up(self, event):
        ...
        end_x = min(max(event.x, 0), self.image_width)  # Begrenzen auf die Bildbreite
        end_y = min(max(event.y, 0), self.image_height)  # Begrenzen auf die Bildhöhe

        # Startkoordinaten ebenfalls beschränken, falls außerhalb
        self.start_x = min(max(self.start_x, 0), self.image_width)
        self.start_y = min(max(self.start_y, 0), self.image_height)

        # Normalisieren der Koordinaten auf das Bild
        x_center = (self.start_x + end_x) / 2 / self.image_width
        y_center = (self.start_y + end_y) / 2 / self.image_height
        box_width = abs(end_x - self.start_x) / self.image_width
        box_height = abs(end_y - self.start_y) / self.image_height

        # Speichern der Bounding Box Koordinaten mit der zugehörigen Klasse und Farbe
        label = (self.current_class, x_center, y_center, box_width, box_height)
    ...
    def save_labels(self):
        ...
        with open(label_path, "w") as f:
            for label in self.labels:
                class_name, x_center, y_center, box_width, box_height = label
                class_id = self.class_buttons.index(class_name)  # Klassen ID bestimmen
                f.write(f"{class_id} {x_center:.6f} {y_center:.6f} {box_width:.6f} {box_height:.6f}\n")  # Klassen ID als int, Koordinaten formatiert
    ...
    def get_class_color(self, class_index):
        ...
        colors = ['blue', 'red', 'yellow', 'orange', 'red', 'brown']  # Farben für vier Klassen
